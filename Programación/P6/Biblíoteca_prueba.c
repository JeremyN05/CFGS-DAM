...
void Show_books(BOOK * Books){ //Es un puntero de BOOK a Books, poque queremos mostrar o modificar los datos que Books contiene.
	...
	printBOOK(&Books[i]); //Este printBOOK, es la variable creada anteriormente, el cual nos ahorra tener que estar escribiendo printf todo el rato, el &Books[i] nos indica que imprima todo lo que haya dentro de la dirección de memoria de Books el & accede a la dirección de memoría de Books.
	}
